...
- **Sidebar (`288px`, cream)**: Each control wrapped in a `Section` with mono 0.65rem 700 uppercase muted label
  - **Layout (mode)**: Connected pill row — `Grid` / `Row` / `Column`. `Row`/`Column` switch into the infinite-stack modes (`hstack` / `vstack`); `Grid` is the original fixed-aspect grid. Switching modes clears loaded images.
  - **Image count / Aspect ratio / Gap / Export size**: Connected pill row (no gap; 1px outer border, internal 1px dividers). Active segment fills lime, inactive segments hover to `var(--bg-secondary)` and `var(--fg)` text. Image count and the Grid presets are hidden in stack modes; Export size relabels to `Export height` (row) / `Export width` (column)
  - **Aspect ratio**: Grid mode uses the 5 fixed ratios. Stack modes use the same pills prefixed with `Auto` (`STACK_ASPECT_RATIOS`) — `Auto` keeps each tile at its image's natural ratio (default), a fixed ratio makes every tile uniform with cover-crop + pan/zoom. Grid `aspectRatio` and stack `stackAspect` are separate state so neither leaks into the other
  - **Grid presets**: Wrap-grid of mini layout previews — 52px wide thumbnails with the layout's blocks rendered as muted/lime rectangles inside; active card border + bg flip to lime (grid mode only)
...
- **Stack modes (`Row` / `Column`)**: `StackCanvas` replaces the grid canvas. Images keep their natural aspect ratio and concatenate left→right (row) or top→bottom (column) at a shared cross-axis size, so the strip grows with each image (the "infinite" stack). Cross axis fits the pane but caps at 480px so a single tall/wide image can't dominate; the strip scrolls on its long axis (`safe center` keeps the start reachable). Cells = the filled images plus one trailing empty drop tile that spawns the next tile on upload; Remove compacts the strip. A fixed `stackAspect` makes every tile that uniform shape (cover-crop); `Auto` sizes each tile to its image. Each cell supports the same drag-to-pan / scroll-to-zoom as grid blocks (in `Auto` at 100% the full image shows; a fixed ratio or zooming crops within). Export builds a `<canvas>` sized to the summed strip with the chosen `Export height`/`width` as the cross axis, baking in each tile's scale/offset
- **Image block**:
