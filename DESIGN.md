...
- **Sidebar (`288px`, cream)**: Each control wrapped in a `Section` with mono 0.65rem 700 uppercase muted label
  ...
  - **Image count / Aspect ratio / Gap / Export size**: Connected pill row (no gap; 1px outer border, internal 1px dividers). Active segment fills lime, inactive segments hover to `var(--bg-secondary)` and `var(--fg)` text. Image count, Aspect ratio and the Grid presets are hidden in stack modes; Export size relabels to `Export height` (row) / `Export width` (column)
