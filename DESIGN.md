...
- **Sidebar (`288px`, cream)**: Each control wrapped in a `Section` with mono 0.65rem 700 uppercase muted label
  - **Layout (mode)**: Connected pill row — `Grid` / `Row` / `Column`. `Row`/`Column` are the infinite-stack modes (`hstack` / `vstack`): an unbounded number of images divide the fixed-aspect canvas into equal columns/rows; `Grid` is the original fixed preset layouts. Switching modes clears loaded images.
